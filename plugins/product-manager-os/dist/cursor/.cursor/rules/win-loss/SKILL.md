...
**Delegate to the `researcher` subagent.** Send the deal notes to `researcher` — wins and losses in **separate** invocations, so neither read is anchored by the other. Ask it to tag every item `stated` or `behavioural`; the whole analysis turns on keeping those apart. If subagents aren't available on this host, run the step yourself and say so — the output is still useful, just less independent.
