...
> From **The Product Channel** by Sid Saladi. This is a drop-in OS — copy this folder into any project (or install it as a plugin) and Cursor starts working like a senior PM partner.
...
**`getskills`** — 3,000+ installable Cursor skills, incl. an 8-pack PM starter set:
- `search_skills({ query })` / `get_skill({ slug })` — find and inspect a skill.
- `install_skill({ slug })` — fetches a skill's full body. (The one-click install writes into Claude's skills directory; on Cursor you save the returned body into `.cursor/rules/` yourself.)
