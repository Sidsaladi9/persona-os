...
tier: guided
time: 60-90 min
inputs: which competitors, and what decision this informs
outputs: research/competitive-<competitor>.md
---
...
**Delegate to the `researcher` subagent.** Send each competitor to a separate `researcher` invocation. Read together, competitors blur into a feature grid; read independently, the structural differences stay visible. If subagents aren't available on this host, run the step yourself and say so — the output is still useful, just less independent.
