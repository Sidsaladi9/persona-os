...
**Run the `researcher` brief from `WORKERS.md`.** This host has no subagents, so you cannot get a genuinely independent read inside this session. Do it anyway — the brief is worth following — but **label the result a self-review, not an independent critique**, and tell the user how to get the real thing: a fresh session with the artifact pasted in alone. Hand the raw request list to `researcher` first. It returns each request with its source, account, and verbatim ask, plus the distribution across accounts — so you triage on how widely something is wanted, not on who wrote the longest ticket.
