...
**Run the `critic` brief from `WORKERS.md`.** This host has no subagents, so you cannot get a genuinely independent read inside this session. Do it anyway — the brief is worth following — but **label the result a self-review, not an independent critique**, and tell the user how to get the real thing: a fresh session with the artifact pasted in alone. You are the author's own agent attacking the author's own document — the least credible critic available. Hand the artifact to `critic` **verbatim and alone**, with none of the conversation that produced it, and use what comes back as your findings base. Then add what only you can add: what you know from `memory/` and `workspace/` that the cold reader couldn't.
