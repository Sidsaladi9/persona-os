...
> The files above start as **templates**. Run `/setup` to fill them in fast, or let Claude fill them in as you work. **Skipped setup?** `/setup` picks up where you left off — it only asks what's still blank — and `/setup status` shows you the gaps first. Either way, every session starts with context instead of questions. Add new files as you accumulate knowledge (e.g. `product-<slug>.md` for a second product), and add a one-line pointer here for each.

## Behavior — what you've been doing (the self-improvement layer)

- [Onboarding](onboarding.md) — how much the OS knows about you, what's still missing, and whether setup was offered/skipped. `/setup status` reads this.
- [Activity log](activity-log.md) — one line per meaningful task; how the OS spots work you repeat. Local-only.
- [Incidents](incidents.md) — output you couldn't use, and why. Read at 1×, not 3× — one real failure is enough to act on.
