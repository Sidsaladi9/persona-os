...
> From **The Product Channel** by Sid Saladi. This is a drop-in OS — copy this folder into any project (or install it as a plugin) and the agent starts working like a senior PM partner.
...
**Flagship workflows.** (This host has no native slash commands — the user invokes these by name, e.g. *"run the weekly workflow"*. Full definitions are in `COMMANDS.md`.)

**Workflows.** For common multi-skill jobs, the OS ships commands that chain skills in the right order — use them when the user invokes them, or suggest them when a request matches:
...
**`getskills`** — 3,000+ installable the agent skills, incl. an 8-pack PM starter set:
- `search_skills({ query })` / `get_skill({ slug })` — find and inspect a skill.
- `install_skill({ slug })` — fetches a skill's full body. (The one-click install writes into Claude's skills directory; on the agent you save the returned body into `skills/` yourself.)
