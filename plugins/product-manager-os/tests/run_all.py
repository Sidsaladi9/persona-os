...
from score_skill import score_file, DIM_NAMES  # noqa: E402
...
def main():
    ...
    dirs = [d for d in sorted(os.listdir(os.path.join(ROOT, "skills")))
            if os.path.isdir(os.path.join(ROOT, "skills", d))]
    missing = [d for d in dirs if not os.path.isfile(os.path.join(ROOT, "skills", d, "SKILL.md"))]
    for d in missing:
